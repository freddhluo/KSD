# Comparison of tests
# for Constant Mean Constant Variance Normality
# dimension = d
...
# Output: p-value
KSD_HZ_Nres = function(Y, g. = g, KSD_new. = KSD_new, henze. = henze, nB. = nB, n. = n){
  ################ Estimation #################
  #Estimated Mean
  mu_hat = colMeans(Y)
  #Estimated Sigma
  ...
  T_obs = KSD_new.(x = t(X), score_function = "gaussian", width = -1)
  ...
  for (b in 1:nB.){
    ...
    # set.seed(b+1)
    ita.N = rmvnorm(n., mean = rep(0,d), sigma = diag(d), method = "chol")
    # Recover Ys_b (n x d)
    ...
    T_b = KSD_new.(x = t(X_b), score_function = "gaussian", width = -1)
    T_b = as.numeric(T_b)
    sample = c(sample, T_b)
    
    HZ_b = henze.(X_b)
    sample_HZ = c(sample_HZ, HZ_b)
  }
  
  pval_KSD = sum((sample - T_obs) > 0) / nB.
  pval_HZ  = sum((sample_HZ - HZ_obs) > 0) / nB.
  return(c(pval_KSD, pval_HZ))
}
...
func = function(cnt){
  ...
  # temp1s = Ys[,1] - mu_hats[1]
  # temp2s = Ys[,2] - mu_hats[2]
  # temp3s = Ys[,3] - mu_hats[3]
  # temp4s = Ys[,4] - mu_hats[4]
  # temp5s = Ys[,5] - mu_hats[5]
  # temps = cbind(temp1s,temp2s,temp3s,temp4s,temp5s)
  # Xs = msqrt(sigma_hats)$invsqrt %*% t(temps)
  
  # temp1p = Yp[,1] - mu_hatp[1]
  # temp2p = Yp[,2] - mu_hatp[2]
  # tempp = cbind(temp1p,temp2p)
  # Xp = msqrt(sigma_hatp)$invsqrt %*% t(tempp)
  ################### Skew / Kurt Test ###################
  ...
  # # Xs_m = Xs - Xs.mu
  # # g.Xs_m = Xs_m^3
  # # JB1_s_m = sum((apply(g.Xs_m, 1, sum))^2) / (6 * n)
  # # h.Xs_m = Xs_m^4 - 6 * Xs_m^2 + 3
  # # JB2_s_m = sum(h.Xs_m) / sqrt(24 * n * d)
  # # pval_s_JB_m = 1 - pchisq(JB1_s_m + JB2_s_m^2, d + 1)
  # # 
  # # Xp.mu = apply(Xp, 1, mean)
  # # Xp_m = Xp - Xp.mu
  # # g.Xp_m = Xp_m^3
  # # JB1_p_m = sum((apply(g.Xp_m, 1, sum))^2) / (6 * n)
  # # h.Xp_m = Xp_m^4 - 6 * Xp_m^2 + 3
  # # JB2_p_m = sum(h.Xp_m) / sqrt(24 * n * d)
  # # pval_p_JB_m = 1 - pchisq(JB1_p_m + JB2_p_m^2, d + 1)
  ################### KSD Nres Test ###################
  pval_s_KSD = KSD_HZ_Nres(Ys)[1]
  pval_p_KSD = KSD_HZ_Nres(Yp)[1]
  
  # plist_size_KSD = c(plist_size_KSD, pval_s_KSD)
  # plist_power_KSD = c(plist_power_KSD, pval_p_KSD)
  ################### Henze Test ######################
  pval_s_Henze = KSD_HZ_Nres(Ys)[2]
  pval_p_Henze = KSD_HZ_Nres(Yp)[2]
  # ################### Bai Test ###################
  # mu2_1s = rep(mu_hats[2], n) + sigma_hats[2,1] / sigma_hats[1,1] * (Ys[,1] - rep(mu_hats[1],n))
  # sigma2_1s = sigma_hats[2,2] - sigma_hats[1,2]^2 / sigma_hats[1,1]
  # X1s = (Ys[,1] - mu_hats[1]) / sqrt(sigma_hats[1,1])
  # X2s = (Ys[,2] - mu2_1s) / sqrt(sigma2_1s)
  # U1s = pnorm(X1s, 0, 1)
  # U2s = pnorm(X2s, 0, 1)
  # 
  ...
  ################### return ###################
  return(c(pval_s_skew, pval_p_skew,
