# Comparsion_SN_GARCH.R
# Multivariate Tests for CCC-GARCH(1,1) model with SN innovations
...
func = function(cnt){
  ...
  while (sign == 0){
    ...
    while (b <= nB){
      ...
      # Recover Ys_b (n x d)
      Ys_b = t(GARCH_d.('SN', as, Bs, Gs, Rs))
      Yp_b = t(GARCH_d.('SN', ap, Bp, Gp, Rp))
      ###### Estimation ######
...
# write.csv(res, file = paste('GARCH_SN_N', '0', lambda, sep = '_'), row.names = F)
...
write.csv(res, file = paste('GARCH_SN_logN', lambda, sep = "_"))
...
stopCluster(cl)
# stopImplicitCluster()
write.csv(result, file = paste('GARCH_SN_logN', lambda, sep = "_"))
