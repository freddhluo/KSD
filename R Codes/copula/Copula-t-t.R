...
KSD_Nres = function(Y, g. = g, KSD_new. = KSD_new, nB. = nB, n. = n){
  ...
  # for testing normal-copula with t margins
  # sqx = function(x){
  #   rho = rho_hat
  #   sqx1 = (rho * dt(x[1],nu_1) * qnorm(pt(x[2],nu_2))) /
  #       (dnorm(qnorm(x[1]))) + (nu_1 + 1) * x[1] / (nu_1 + x[1]^2)
  #   sqx2 = (rho * dt(x[2],nu_2) * qnorm(pt(x[1],nu_1))) /
  #       (dnorm(qnorm(x[2]))) + (nu_2 + 1) * x[2] / (nu_2 + x[2]^2)
  #   return(c(sqx1, sqx2))
  # }
...
DGP = function(cop){
  ...
  for (t in 2:(n + ncut)){
    Y1[t] = delta0[1] + alpha0[1] * Y1[t-1] + 
      sigma0[1] / sqrt(2) * qt(p = cop[t,1], df = nu_1) + 
      beta0[1] * sigma0[1] / sqrt(2) * qt(p = cop[t-1,1], df = nu_1)
    Y2[t] = delta0[2] + alpha0[2] * Y2[t-1] + 
      sigma0[2] / sqrt(1.5) * qt(p = cop[t,2], df = nu_2) + 
      beta0[2] * sigma0[2] / sqrt(1.5) * qt(p = cop[t-1,2], df = nu_2)
  }
...
DGP_b = function(coef1, coef2, rho){
  ...
  for (t in 2:(n + ncut)){
    Y1[t] = delta[1] + alpha[1] * Y1[t-1] + 
      sigmahat[1] / sqrt(2) * qt(p = cop[t,1], df = nu_1) + 
      beta[1] * sigmahat[1] / sqrt(2) * qt(p = cop[t-1,1], df = nu_1)
    Y2[t] = delta[2] + alpha[2] * Y2[t-1] + 
      sigmahat[2] / sqrt(1.5) * qt(p = cop[t,2], df = nu_2) + 
      beta[2] * sigmahat[2] / sqrt(1.5) * qt(p = cop[t-1,2], df = nu_2)
  }
  Y = cbind(Y1[(ncut+1):(n+ncut)], Y2[(ncut+1):(n+ncut)])
